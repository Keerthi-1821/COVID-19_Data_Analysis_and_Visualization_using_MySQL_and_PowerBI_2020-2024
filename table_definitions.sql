...
queries that filter or sort records based on the columns in covid_deaths table and covid_vacc2 table */
 
CREATE INDEX idx_date ON covid_deaths (date_occured);
CREATE INDEX idx_location_total_cases_deaths ON covid_deaths (location, total_cases, total_deaths);

CREATE INDEX idx_location_total_vaccinations_new ON covid_deaths (location, total_vaccinations, new_vaccinations);
